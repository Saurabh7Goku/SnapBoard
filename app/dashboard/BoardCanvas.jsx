...
export const BoardCanvas = ({ elements, currentBoard, user }) => {
    ...
    const renderLatex = (text) => {
        ...
        try {
            // Fallback: If no $ delimiters, treat whole as inline math (for formula cards)
            if (!processed.includes('$')) {
                try {
                    const rendered = katex.renderToString(processed.trim(), {
                        displayMode: false,  // Inline only
                        throwOnError: false,
                        output: 'html'
                    });
                    return rendered;  // No extra div wrapper for inline
                } catch (e) {
                    console.error('KaTeX fallback error:', e);
                    return `<span class="text-red-500">${processed}</span>`;  // Highlight errors
                }
            }

            // Inline math ($...$) only - strip any $$ and treat as inline
            processed = processed.replace(/\$\$([\s\S]*?)\$\$/g, (match, latex) => {
                // Convert $$ to $ by stripping outer $ and rendering inline
                const innerLatex = latex.trim().replace(/^\$+|\$+$/g, '');
                try {
                    const rendered = katex.renderToString(innerLatex, {
                        displayMode: false,
                        throwOnError: false,
                        output: 'html'
                    });
                    ...
                    console.error('KaTeX converted display error:', e);
            ...
            // Bold (**text**) - applied after math to avoid conflicts
    ...
    const renderElement = useCallback((el) => {
        ...
        return (
            <div
                ...
                onMouseEnter={(e) => {
                    if (el.type === 'formula') {
                        const latexEl = e.currentTarget.querySelector('.formula-display');
            ...
            >
                ...
                {el.type === 'formula' ? (
                    <div className="h-[calc(100%-60px)] flex flex-col">
                        <div
                            className="formula-display bg-white/70 backdrop-blur rounded-lg p-2 mb-2 text-center text-sm overflow-hidden flex-grow text-black"
                            style={{ minHeight: '40px', flex: '0 0 auto' }}
                            dangerouslySetInnerHTML={{ __html: renderLatex(el.latex) }}
                        />
                        <input
                            type="text"
                            value={el.latex}
                            ...
                            className="w-full px-2 py-1 text-xs bg-white/50 backdrop-blur rounded border border-white/30 mb-2 text-gray-800"
                        />
                        <div className="flex gap-2 mb-2 flex-wrap">
                            <input
                                ...
                                className="px-2 py-0.5 text-xs bg-blue-200/50 backdrop-blur rounded-full flex-1 min-w-0 text-gray-800"
                                placeholder="Subject"
                            />
                            <input
                                type="text"
                                ...
                                className="px-2 py-0.5 text-xs bg-green-200/50 backdrop-blur rounded-full flex-1 min-w-0 text-gray-800"
                                placeholder="Topic"
                            />
                        </div>
                        {el.height >= 180 && (
                            <textarea
                                ...
                                placeholder="Notes..."
                                className="w-full px-2 py-1 text-xs bg-white/50 backdrop-blur rounded border border-white/30 resize-none overflow-hidden mt-1 text-gray-800"
                                style={{ height: 'auto', minHeight: '1.5rem', maxHeight: '150px', fontFamily: 'Caveat, cursive', lineHeight: '1' }}
                                onInput={e => {
                    ...
                    <>
                        <textarea
                            ...
                            className="w-full h-[calc(100%-40px)] px-2 py-1 bg-white/50 backdrop-blur rounded border border-white/30 resize-none text-black"
                        ...
                        <textarea
                            ...
                            onChange={e => updateElement(el.id, { notes: e.target.value })}
                            placeholder="Notes..."
                            className="w-full px-2 py-1 text-xs bg-white/50 backdrop-blur rounded border border-white/30 resize-none overflow-hidden mt-1 text-black"
                            style={{ height: 'auto', minHeight: '1.5rem', maxHeight: '150px', fontFamily: 'Caveat, cursive', lineHeight: '1' }}
                            onInput={e => {
                                const textarea = e.target;
                                textarea.style.height = 'auto';
                                    ...
                                    {el.columns.map((col, i) => (
                                        <th key={i} className="px-2 py-2 border-b border-gray-300">
                                            <input
                                                ...
                                                className="w-full bg-transparent text-center font-semibold focus:outline-none focus:ring-2 focus:ring-indigo-400 rounded-md px-1 py-1"
                                            />
                                        </th>
                                    ))}
                                </tr>
                                        ...
                                        {row.cells.map((cell, j) => (
                                            <td key={j} className="px-4 py-2 border-b border-gray-200 text-center">
                                                <input
                                                    ...
                                                    className="w-full bg-transparent text-center focus:outline-none focus:ring-2 focus:ring-indigo-300 rounded-md px-1 py-1"
                                                />
                                            </td>
                                        ))}
                                    </tr>
                    ...
                    <div className="h-[calc(100%-40px)] flex flex-col">
                        ...
                        <textarea
                            ...
                            onChange={e => updateElement(el.id, { notes: e.target.value })}
                            placeholder="Notes..."
                            className="w-full px-2 py-0 text-xs bg-white/50 backdrop-blur rounded border border-white/30 resize-none mt-2 text-gray-800"
                            rows={2}
                        />
                    </div>
                )}
    ...
    return (
        <div className="w-full h-full pt-20 overflow-auto" onMouseMove={handleGlobalMove} onMouseUp={() => { setDraggedId(null); setResizingId(null); }}>
            ...
            {hoveredFormula && (
                <div
                    className="fixed z-[9999] bg-white rounded-xl shadow-2xl p-6 border-2 border-purple-500 max-w-md w-full max-h-[80vh] overflow-auto"
